...
#| output: false
#| echo: false
# Custom functions for parameter tables and inference tables
using DataFrames
using Typst_jll

η_coefficient_of_variation(fpm::Pumas.AbstractFittedPumasModel) =
    _η_coefficient_of_variation(fpm.model, coef(fpm), Pumas.getvvrandeffsorth(fpm))

function _η_coefficient_of_variation(
    m::PumasModel,
    param::NamedTuple,
    vvrandeffsorth::AbstractVector,
)
    rfxs = m.random(Pumas.EMPTY_SUBJECT, param)
    normal_rfxs = ParamSet(
        Pumas.ntfilter(
            x -> x isa Union{Normal,MvNormal,LogNormal,MvLogNormal},
            rfxs.params,
        ),
    )
    if length(normal_rfxs.params) != length(rfxs.params)
        names_nonnormal_rfxs = keys(Base.structdiff(rfxs.params, normal_rfxs.params))
        @warn(
            "η_coefficient_of_variation cannot be computed for $(join(names_nonnormal_rfxs, ", ", " and ")): only random effects of type `Normal`, `MvNormal`, `LogNormal` or `MvLogNormal` are supported"
        )
    end

    # Early exit if there are no `Normal` or `MvNormal` random effects
    if isempty(normal_rfxs.params)
        return (;)
    end

    vtrandeffs = map(
        NamedTuple{keys(normal_rfxs)} ∘
        Pumas.TransformVariables.transform(Pumas.totransform(rfxs)),
        vvrandeffsorth,
    )
    cv =
        map(NamedTuple{keys(normal_rfxs)}(keys(normal_rfxs)), normal_rfxs.params) do k, rfx
            return sqrt.(exp.(std(map(Base.Fix2(getfield, k), vtrandeffs))) .- 1)
        end
    return cv
end
...
function _infer_footnote(inf)
    if inf.vcov isa Pumas.Bootstraps
        strat_string = if isempty(inf.vcov.stratify_by)
            "without stratified resampling"
        else
            "with stratified resampling based on $(inf.vcov.stratify_by)"
        end
    ...
    elseif inf.vcov isa Pumas.PumasFailedCovariance
        "not available; asymptotic variance-covariance matrix estimation failed."
    else
        throw(ErrorException("Simulated inference not supported"))
    end
end
"""
    _unpack_entries_omega(nt::NamedTuple, restrict = nothing)

`restrict` controls which entries are included in the output:
- `nothing` — include everything
- `(:Ω, :Ωvc)` (tuple/vector of Symbols) — include all elements of the named parameters
- `Dict(:Ω => [1, 3])` — include only specific diagonal indices per parameter
"""
function _unpack_entries_omega(nt::NamedTuple, restrict = nothing)
    # Normalize tuple/vector of symbols to a Dict with no index restriction
    if restrict isa Union{Tuple{Vararg{Symbol}},AbstractVector{Symbol}}
        restrict = Dict(k => nothing for k in restrict)
    end
    mapping = Pair{String,Any}[]
    subscript(s) = join(Char(0x2080) + Int(c) for c in s)
    for (k, v) in pairs(nt)
        if isnothing(restrict) || k in keys(restrict)
            if v isa Number
                push!(mapping, "$k" => v)
            else
                for (i, item) in enumerate(v)
                    idx_filter = isnothing(restrict) ? nothing : restrict[k]
                    if isnothing(idx_filter) || i in idx_filter
                        sub = join(subscript.(reverse(digits(i))))
                        push!(mapping, string(k, sub, ",", sub) => item)
                    end
                end
            end
        end
...
CI_label(fpmi) = string(
    ...
    round(100 * fpmi.level, sigdigits = 4),
    "% confidence level",
)
cond_info(
    fpmi::Pumas.FittedPumasModelInference{<:Any,<:Pumas.PumasFailedCovariance,<:Any};
    correlation,
) = nothing
cond_info(
...
"""
    parameter_table(fpmi; eta_omega::Union{Nothing,Dict{Symbol,Symbol}}, cv_vars, correlation_condition = true, include_se = true, include_rse = true, include_shrinkage = true)

`eta_omega` must be a `Dict{Symbol,Symbol}` mapping random effect names to their
variance parameter names, e.g. `Dict(:η => :Ω)` or `Dict(:η => :Ω, :κ => :Ω_IOV)`.
Pass `nothing` to omit shrinkage and CV columns.
"""
function parameter_table(fpmi; eta_omega::Union{Nothing,Dict{Symbol,Symbol}}, cv_vars, correlation_condition = true, include_se = true, include_rse = true, include_shrinkage = true)

    _footnotes = []
    coff = coeftable(fpmi)
    if fpmi.vcov isa Pumas.PumasFailedCovariance
        coff.se .= missing
        coff.relative_se .= missing
        coff.ci_lower .= missing
        coff.ci_upper .= missing
    end
    descriptions = Dict(Pumas._markdown_descriptions_params(fpmi.fpm.model))

    insertcols!(coff, 2, :description => [descriptions[p] for p in coff.parameter])
    ...
    combine_ci!(coff)
    colmetadata!(coff, :CI, "label", string(100 * fpmi.level, "% ", "CI"), style = :note)

    # Column abbreviation definitions
    _abbrevs = ["CI: Confidence Interval"]

    has_shrinkage = !isnothing(eta_omega) && include_shrinkage
    # Munge shrinkage
    if has_shrinkage
        ηmap = ηshrinkage(fpmi.fpm)
        ηmap = _unpack_entries_omega(
            NamedTuple{replace(keys(ηmap), eta_omega...)}(values(ηmap)),
        )
        ηdf = DataFrame(parameter = first.(ηmap), shrinkage = last.(ηmap))
        ηdf.shrinkage .*= 100

        # Add Shrinkage
        df_sh1 = leftjoin(coff, ηdf; on = :parameter, order = :left)
        colmetadata!(df_sh1, :shrinkage, "label", "Shrinkage [%]", style = :note)
        push!(_abbrevs, "Shrinkage: η shrinkage")
    else
        df_sh1 = coff
    end

    has_cv = !isnothing(cv_vars)
    # Munge CV
    if has_cv
        ηmap = η_coefficient_of_variation(fpmi.fpm)
        ηmap = _unpack_entries_omega(
            NamedTuple{replace(keys(ηmap), eta_omega...)}(values(ηmap)),
            cv_vars,
        )
        ηdf = DataFrame(parameter = first.(ηmap), CV = last.(ηmap))
        ηdf.CV .*= 100

        # Add CV
        df_sh1 = leftjoin(df_sh1, ηdf; on = :parameter, order = :left)
        colmetadata!(df_sh1, :CV, "label", "CV [%]", style = :note)
        push!(_abbrevs, "CV: η coefficient of variation")
    end
    ...
    if include_se
        push!(_abbrevs, "SE: Standard Error")
        colmetadata!(df_sh1, :se, "label", "SE", style = :note)
    else
        select!(df_sh1, Not(:se))
    end

    df_sh1.relative_se .*= 100
    if include_rse
        push!(_abbrevs, "RSE: Relative Standard Error")
        colmetadata!(df_sh1, :relative_se, "label", "RSE [%]", style = :note)
    else
        select!(df_sh1, Not(:relative_se))
    end

    # Build footnotes: abbreviations → inference method → divider → metrics
    push!(_footnotes, join(_abbrevs, " • "))

    _metrics = String[]
    cond_fpmi = cond_info(fpmi; correlation = correlation_condition)
    if !isnothing(cond_fpmi)
        corr_label = correlation_condition ? "correlation" : "covariance"
        push!(_metrics, "Condition number ($corr_label): $(round(cond_fpmi, sigdigits = 6))")
    end
    push!(_metrics, "Log-likelihood: $(round(loglikelihood(fpmi.fpm), sigdigits = 6))")
    epsshrinkage = ϵshrinkage(fpmi.fpm)
    if !isempty(epsshrinkage)
        eps_parts = [string(k, " = ", round(v * 100, sigdigits = 4), "%") for (k, v) in pairs(epsshrinkage)]
        push!(_metrics, "ϵ-shrinkage: " * join(eps_parts, ", "))
    end
    push!(_footnotes, join(_metrics, " • "))
...
function infer_table(
    ...
    round_mode = :auto,
    eta_omega = nothing,
    cv_vars = nothing, # needs to generate names of parameters for rows to calculate cv for
    ...
    coff = parameter_table(fpmi; eta_omega, cv_vars, include_se, include_rse, include_shrinkage)
    partable_footnotes = metadata(coff, "footnotes")
    constant_to_annotated!(coff)
    max_text_col = 0
    if include_param_name
        ...
        select!(coff, Not(:parameter))
    end
    if include_descriptions
        _description = @. Cell(coff.description; halign = :left)
        if include_param_name
            rest = hcat(rest, _description)
        else
            rest = [_description;;]
        end
        max_text_col += 1
    else
        select!(coff, Not(:description))
    end
    ...
    if max_text_col == 0
        ...
        rest = hcat(rest, _para)
    end

    if include_se
        rest = hcat(rest, @. Cell(coff.se))
    end
    if include_rse
        rest = hcat(rest, @. Cell(coff.relative_se))
    end

    rest = hcat(rest, @. Cell(coff.CI))

    if include_shrinkage && !isnothing(eta_omega)
        rest = hcat(rest, @. Cell(coff.shrinkage))
    end

    if !isnothing(cv_vars)
        _CV = @. Cell(coff.CV)
        rest = hcat(rest, _CV)
    end
    ...
    source_footnotes = []
    !isnothing(caller_path) && push!(source_footnotes, "Program Source: $(caller_path)")
    !isnothing(path) && push!(source_footnotes, "Source: $(path)")

    header = [
        Cell(h; halign = i > max_text_col ? :center : :left) for
        (i, h) in enumerate(header_row)
    ]

    tab = Table(
        [header; rest];
        header = 1,
        footnotes = vcat(partable_footnotes, source_footnotes),
        round_digits,
        round_mode,
...
function combine_ci!(df)
    df.CI = map(df.ci_lower, df.ci_upper) do l, u
        any(ismissing, (l, u)) ? Concat(missing, ", ", missing) : Concat(l, ", ", u)
    end
    select!(df, Not(:ci_lower, :ci_upper))
    df
end

function constant_to_annotated!(df)
    annotate(x) = Annotated(x, "Fixed to starting values during estimation", label = "†")
    df.estimate = map(x -> x[1] ? annotate(x[2]) : x[2], zip(df.constant, df.estimate))
    select!(df, Not(:constant))
end

function table_to_typst(name::AbstractString, directory, table)
    typfile = joinpath(directory, name)
...
Our journey begins with a CSV file exported from the clinical database. Let's load it and take our first look:
...
A comprehensive list is provided in the [Data Representation Tutorial](https://tutorials.pumas.ai/html/LearningPaths/02-LP/04-Module/T1-DataRepresentation.html#how-read_pumas-builds-a-population){target="_blank"}.
...
Each function produces focused plots for specific diagnostics.
...
VPC is a simulation-based diagnostic comparing observed data quantiles to model-predicted quantile intervals. Agreement indicates adequate model predictive performance. We use prediction correction to correct for the heterogeneity in covariates and the dose amounts. 

```{julia}
foce_vpc = vpc(foce_fit; covariates = [:tad], prediction_correction = true)
```

The `covariates = [:tad]` argument bins observations by time after dose. The option `prediction_correction=true` means that we produce a pcVPC to handle the difference in covariates and dose amounts. The `vpc()` function simulates many replicates of the dataset and computes prediction intervals for the 5th, 50th, and 95th percentiles.
...
```{julia}
#| output: false
foce_inference_2cmt = infer(foce_fit_2cmt)
```

We can also display this as a table:

```{julia}
inf_table = infer_table(foce_inference_2cmt; eta_omega=Dict(:η=>:Ω_pk,), cv_vars=(:Ω_pk,), include_descriptions=false)
```
```{julia}
#| output: false
table_to_typst("tab1-infer.typ", joinpath(@__DIR__, "figures"), inf_table)
```
...
- **Log-Likelihood Ratio Test**: For nested models, compute LRT = -2 × (LL<sub>reduced</sub> - LL<sub>full</sub>), which follows a χ² distribution with degrees of freedom equal to the difference in number of parameters
:::
...
#### Diagnosing Covariates Based on Empirical Bayes Estimates (✓)
...
#| tbl-cap: "Parameter estimates with standard errors and 95% confidence intervals for covariate model. Covariate parameters (θsexCLF for sex effect on clearance, θvcWEIGHTB for weight effect on volume) with confidence intervals excluding neutral values indicate statistically significant covariate relationships."
...
## A.5 Parameter Uncertainty Quantification {#sec-uncertainty}
...
coefs_covar = coeftable(infer_covar)
```


---

## A.6 Pharmacodynamic Modeling {#sec-pkpd}
...
**INH = I<sub>max</sub> × Ce<sup>γ</sup> / (IC<sub>50</sub><sup>γ</sup> + Ce<sup>γ</sup>)**
...
### Simultaneous Multi-Endpoint PKPD Modeling
...
#### Fitting the Simultaneous PKPD Model
...
### Sequential PKPD Modeling with Parameter Transfer
...
#| tbl-cap: "Comparison of PD parameter estimates between simultaneous (pd_ft) and sequential (ft_seq) modeling approaches. Estimates should be similar but not identical due to different uncertainty propagation. Sequential approach treats PK parameters as fixed, while simultaneous approach allows PKPD parameter interactions during estimation."
...
## A.7 Simulation for Decision Support {#sec-simulation}
...
addl = 5
ii = 24
rng_simulation = Random.seed!(15438)
...
dfproccessed=@chain simdf begin
    ...
    combine(groupby(_, [:DOSE, :rep_id, :id])) do gdf
        ...
        i = findfirst(x -> tr_low ≤ x < tr_high, gdf.PD_CONC)
        ...
        _ssdf = filter(df -> df.time >= 4*24 && tr_low ≤ df.PD_CONC < tr_high, gdf)
...
table_to_typst("tab2-simtable.typ", joinpath(@__DIR__, "figures"), simulation_table)
simulation_table
```
...
## A.8 Installation and Setup {#sec-installation}
...
**Installation instructions**: [https://docs.pumas.ai/stable/platformdetails/desktop/](https://docs.pumas.ai/stable/platformdetails/desktop/){target="_blank"}
...
**Setup and configuration**: [https://docs.pumas.ai/stable/platformdetails/pumascluster/](https://docs.pumas.ai/stable/platformdetails/pumascluster/){target="_blank"}
...
**For enterprise inquiries**: Contact [sales@pumas.ai](mailto:sales@pumas.ai)

---

## A.9 Additional Resources {#sec-resources}
...
- **Pumas Documentation**: [https://docs.pumas.ai](https://docs.pumas.ai){target="_blank"} — Complete API reference, function documentation, and technical guides
- **Julia Documentation**: [https://docs.julialang.org](https://docs.julialang.org){target="_blank"} — Julia language reference and standard library documentation

### Tutorials and Learning

- **Pumas Tutorials**: [https://tutorials.pumas.ai](https://tutorials.pumas.ai){target="_blank"} — Step-by-step tutorials covering data preparation, modeling, diagnostics, and simulation
- **AskPumas AI Assistant**: [https://ask.pumas.ai](https://ask.pumas.ai){target="_blank"} — AI-powered learning assistant for interactive help with Pumas concepts and code
- **PumasAI YouTube Channel**: [https://www.youtube.com/@pumasai](https://www.youtube.com/@pumasai){target="_blank"} — Video tutorials, webinars, and conference presentations

### Community and Support

- **Pumas Discourse Forum**: [https://discourse.pumas.ai](https://discourse.pumas.ai){target="_blank"} — Community forum for questions, discussions, and sharing workflows
- **Julia Discourse**: [https://discourse.julialang.org](https://discourse.julialang.org){target="_blank"} — General Julia programming community

### Reproducible Research

- **Quarto**: [https://quarto.org](https://quarto.org){target="_blank"} — Scientific publishing system used for this tutorial